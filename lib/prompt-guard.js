...
function neutralizeInjectionSignals(prompt) {
  let out = prompt;
  for (const pattern of INJECTION_PATTERNS) {
    out = out.replace(pattern, "[REDACTED_SYSTEM_OVERRIDE_ATTEMPT]");
  }
  out = out.replace(/(?:\[REDACTED_SYSTEM_OVERRIDE_ATTEMPT\]\s*){2,}/g, "[REDACTED_SYSTEM_OVERRIDE_ATTEMPT] ");
  return out.trim();
}
...
export function preparePromptForGeneration(prompt) {
  ...
  // Neutralize injection phrases so they aren't forwarded to the model
  const neutralizedPrompt = neutralizeInjectionSignals(sanitizedPrompt);
