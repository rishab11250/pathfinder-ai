import "server-only";
import { cachedGenerateGeminiContent, INDUSTRY_INSIGHT_CACHE_TTL_MS, generateCacheKey } from "../cache/index";
import { buildSecurePrompt } from "../ai/prompt-safety";
import { extractJSON } from "../ai/validate";
import { buildIndustryContext } from "../ai/ai-context";

export const INDUSTRY_INSIGHT_TTL_MS = 24 * 60 * 60 * 1000;

function buildIndustryInsightPrompt(industry) {
  return buildSecurePrompt({
    context: buildIndustryContext(industry),
...
function parseInsightResponse(result) {
  ...
  try {
    ...
    const message = `Failed to parse AI response as JSON: ${parseError?.message || parseError}`;
    console.error("[Industry Insights] parseInsightResponse error:", message);
...
export async function generateIndustryInsightData(industry) {
  try {
    ...
    try {
      ...
      console.error(`Gemini estimate fallback failed for ${industry}, returning default fallback:`, estimateError);
