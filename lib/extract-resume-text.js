...
/**
 * PDF extraction (FIXED for Next.js 15 + Webpack)
 */
async function extractPdf(file) {
  // IMPORTANT: use legacy build for compatibility
  const pdfjs = await import("pdfjs-dist/legacy/build/pdf");

  // Worker must also use legacy path
...
export async function extractTextFromFile(file) {
  ...
  try {
    ...
    throw new Error(
      "Failed to read file. If it's scanned PDF, paste text manually."
    );
  }
