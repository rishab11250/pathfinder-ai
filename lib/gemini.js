  ...
  for (const modelName of modelNames) {
    ...
    while (attempt < MAX_RETRIES) {
      try {
        ...
        throw new GeminiError(error.message || "Unknown AI error", "UNKNOWN");
      }
    }
...
export async function generateGeminiContent(prompt, { timeout = 30000 } = {}) {
  return runWithGeminiFallback((model) => model.generateContent(prompt), timeout);
}
