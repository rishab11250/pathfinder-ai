...
import { createErrorResponse } from "../action-helpers/action-errors";
import { logActionError } from "../action-helpers/action-logger";
...
export async function safeFetch(urlString, options = {}, redirectCount = 0) {
  ...
  try {
    ...
    if (!response.body) {
      return createErrorResponse("Response has no body");
    }
