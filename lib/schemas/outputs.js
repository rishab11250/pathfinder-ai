...
export const careerDecisionOutputSchema = z.object({
  optionsAnalysis: z.array(z.object({
    option: z.string(),
    pros: z.array(z.string()).min(1).max(5),
    cons: z.array(z.string()).min(1).max(5),
    riskLevel: z.enum(["High", "Medium", "Low"]),
    shortTermImpact: z.string().min(5).max(300),
    longTermImpact: z.string().min(5).max(300),
    skillsRequired: z.array(z.string()).min(0).max(5)
  })).min(2).max(5),
  recommendedOption: z.string().min(1).max(200),
  reasoning: z.string().min(10).max(1000),
  nextStep: z.string().min(5).max(300),
